...
export function CustomMarkersPanel({
  ...
  const [deleting, setDeleting] = useState<CustomMarker | null>(null);
  const { deleteCustomMarker } = mutations;

  const confirmDelete = async (): Promise<void> => {
    if (!deleting) return;
    try {
      await deleteCustomMarker.mutateAsync(deleting.id);
      setDeleting(null);
    } catch {
  ...
  return (
    <>
            ...
            >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem variant="destructive" onSelect={() => setDeleting(marker)}>
                    <Trash2 /> Delete
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
      ...
      {deleting && (
        <ConfirmDialog
          title="Delete marker?"
          description={`Remove "${deleting.name}" from the map?`}
          pending={deleteCustomMarker.isPending}
