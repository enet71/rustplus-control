...
**Required behavior:** Any `DropdownMenuItem` whose `onSelect` opens a `Dialog`/`AlertDialog` must defer that state change to the next tick instead of calling it synchronously — `components/ui/dropdown-menu.tsx`'s `DropdownMenuItem` wraps every `onSelect` in `setTimeout(() => onSelect(event), 0)`. **That alone was not sufficient**: the user hit the identical unresponsive-buttons symptom again in the delete-confirmation dialog (`ConfirmDialog`, opened the same way from a destructive `DropdownMenuItem`). The other half of the race is `DropdownMenuContent`'s default `onCloseAutoFocus`, which tries to return focus to the (now-hidden) trigger button as the menu closes — this fights the newly-opened dialog's own focus trap. Fixed by also making `DropdownMenuContent` call `event.preventDefault()` in `onCloseAutoFocus` by default (still forwarding to a caller-supplied `onCloseAutoFocus` if one is passed), so focus return doesn't happen at all when a menu item's selection immediately opens a dialog — which is every current usage in this app. Both fixes live in the shared `components/ui/dropdown-menu.tsx`, not per call site.

**Evidence:** User reported "В модалке редактирования метки не работает кенсел и клоз" (Cancel and Close don't work in the marker-edit modal) after the edit flow was wired through a `DropdownMenuItem`; after the `setTimeout` fix alone, user reported the identical failure in the marker-delete confirmation dialog ("не работают кнопки в модалке удаления метки").
